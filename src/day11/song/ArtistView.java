...
public class ArtistView {
    ...
    public static void start() {
        ...
        while (true) {
            System.out.println("\n****** 음악 관리 프로그램 ******");
            System.out.printf("# 현재 등록된 가수: %d 명\n", repository.count());
    ...
    private static void searchArtist() {
        ...
        if (repository.isRegistered(artistName)) { // 검색한 가수가 저장되어있다면
    ...
    private static void registerSong() {
        ...
        if (!repository.isRegistered(artistName)) { // 신규 등록인가?
            ...
            System.out.printf("\n# %s님이 신규 등록되었습니다.\n", artistName);
        } else { // 신규등록 아니라 기존 등록가수에 곡명만 추가하는것인가?

            // 새로운 노래를 추가해본다. 추가가 잘 되었는지 여부를 확인한다.
            boolean flag = repository.addNewSong(artistName, songName);

            if(flag) { // 기존 가수의 리스트에 노래만 추가하는 경우
                System.out.printf("\n# %s님의 노래 목록에 %s곡이 추가되었습니다.", artistName, songName);
            } else { // 노래가 중복된 경우
                System.out.printf("\n# [%s]곡은 이미 등록된 노래입니다.\n", songName);
            }
        }
